...
const OrderStatusManager = ({ orderId, currentStatus }) => {
  ...
  const fetchStatusInfo = async () => {
    try {
      const response = await fetch(`/api/orders/${orderId}/status-info`);
...
const StatusHistory = ({ orderId }) => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        const response = await fetch(`/api/orders/${orderId}/status-flows`);
        const data = await response.json();
...
// 获取状态信息
router.get('/:id/status-info', 
  StatusValidationMiddleware.getOrderStatusInfo, 
  getOrderStatusInfo
);
...
const getOrderStatusInfo = async (orderId) => {
  try {
    ...
    if (result.success) {
      const { currentStatus, availableTransitions, canCancel } = result.data;
      
      // 根据状态信息更新UI
      updateStatusDisplay(currentStatus);
      updateActionButtons(availableTransitions, canCancel);
    }
  } catch (error) {
    console.error('获取状态信息失败:', error);
  }
...
#### 获取状态流转历史
```
GET /api/orders/:id/status-flows
```
